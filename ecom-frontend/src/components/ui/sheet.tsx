	...
	return (
		<SheetPortal>
			...
			<SheetPrimitive.Content
				...
				className={cn(
					'bg-[#0d0f14] border-[var(--border)] text-[var(--text-primary)] data-[state=open]:animate-in data-[state=closed]:animate-out fixed z-50 flex flex-col gap-4 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500',
					side === 'right' &&
...
function SheetHeader({ className, ...props }: React.ComponentProps<'div'>) {
	return (
		<div
			...
			className={cn('flex bg-[var(--bg-elevated)] border-b border-[var(--border)] flex-col gap-1 p-4', className)}
			{...props}
		/>
	);
}
...
function SheetFooter({ className, ...props }: React.ComponentProps<'div'>) {
	return (
		<div
			...
			className={cn('mt-auto flex flex-col gap-2 p-4 bg-[var(--bg-elevated)] border-t border-[var(--border)]', className)}
			{...props}
		/>
	);
}
